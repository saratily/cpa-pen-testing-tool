...
func init() {
	migrations.MustRegisterTx(func(db migrations.DB) error {
		...
		_, err := db.Exec(`INSERT INTO default_tools(type, category, options, format, help, active, can_change, selected)
				...
				('shodan', 'reconnaissance', '{{.ip_address}}', 'shodan host {{.ip_address}}', 'Ffuf is a lightning-fast subdomain fuzzer that’s highly customizable.', 1, 0, 0),
				('ffuf', 'web_enumeration', '{{.domain}}', 'ffuf -u http://{{.domain}}/FUZZ -w /usr/share/wordlists/dirb/common.txt -p 1', '', 1, 0, 0),
				...
				('wfuzz', 'web_enumeration', '{{.URL}}', 'wfuzz -c -w /usr/share/wordlists/dirb/common.txt {{.URL}}/FUZZ', 'wfuzz is one of the most powerful and versatile subdomain fuzzing tools out there. With Wfuzz, you can quickly and easily discover hidden subdomains on a target website, making it an essential tool for any penetration tester or ethical hacker.', 1, 0, 0),
				('Wappalyzer', 'web_enumeration', '{{.URL}}', 'python wappy.py -u {{.URL}}', 'wappalyzer is a browser extension used for identifying the technologies used by websites. It is an excellent tool for web reconnaissance and can be used by both security researchers and web developers.', 1, 0, 0),
